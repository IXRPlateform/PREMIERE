...
def simplify_mesh(vertices, faces, target_ratio):
    mesh = trimesh.Trimesh(vertices=vertices, faces=faces)
    target_faces = int(len(faces) * target_ratio)
    simplified = mesh.simplify_quadric_decimation(face_count=target_faces)
    return simplified.vertices, simplified.faces

def save_obj_with_mtl_indv(obj_filename, mtl_filename, all_verts, all_faces, group_materials, material_colors,all_tracking_ids,obj_res):
    """
    Save an .obj file + .mtl file that references multiple groups (humans). Save this for each of the human
    ...
    # 1) Write the MTL file (shared among all .obj files)
    ...
    # 2) Write separate OBJ files for each mesh
    last_item = os.path.basename(obj_filename)
    dir_path = os.path.dirname(obj_filename)
    filename_base,ext= os.path.splitext(last_item)
   

    
    for i, (verts_i, faces_i,id_number) in enumerate(zip(all_verts, all_faces,all_tracking_ids)):
        obj_file_ind = f"{id_number}_{int(filename_base)}.obj"
        obj_file_i=os.path.join(dir_path,obj_file_ind)
        mat_name = group_materials[i]

        if obj_res != 1.0:
            verts_i,faces_i=simplify_mesh(verts_i,faces_i,obj_res)

        normals_i = compute_vertex_normals(verts_i, faces_i)

        with open(obj_file_i, 'w') as f_obj:
            # Reference the .mtl file
            #f_obj.write(f"mtllib {mtl_filename}\n")
            f_obj.write(f"o Mesh_{i}\n")
...
def _compute_verts_for_person(human_data, model, angle=0, Zoffset=0, useExpression=False, device='cpu', estimateVertices=True):
    ...
    verts = verts + trans
    verts = verts.float()  # Ensure same dtype as R_x
...
def estimateFromSMPLXJ3DWithFloor(human_data, model, angle=0, Zoffset=0, useExpression=False, device='cpu', estimateVertices=False):
    ...
    j3dRot[:, :, 1] += Zoffset
    return j3dRot.detach().cpu().numpy().squeeze(0), t_id
...
"""
def save_obj(humans_in_frame, filename, model, obj_res,angle=0, Zoffset=0,useExpression=False, device='cpu',save_individal_obj=False):
    """
    ...
    if save_individal_obj:
           save_obj_with_mtl_indv(
...
def save_glb(humans_in_frame, filename, model, angle=0, Zoffset=0, useExpression=False, device='cpu',save_individal_glb=False):
    """
    ...
    if save_individal_glb:
        # Save single human mesh of a frame in a single glb file
        last_item = os.path.basename(filename)
        ...
        if humans_in_frame:
            ...
            for idx, human_data in enumerate(humans_in_frame):
                ...
                individual_filename = f"{human_id}_{int(filename_base)}.glb"
                full_filename=os.path.join(dir_path,individual_filename)
                 # Export the mesh to an individual GLB file
        ...
        # Save combined human mesh of a frame in a single glb file
        glb_bytes=None
